...
export function EditTicketModal({ ticket, members, columns, onClose, onSave }: Props) {
  const [form, setForm] = useState({ ...ticket });
  
  function handleSave() {
    if (!form.title.trim()) return;
    onSave({ ...form, storyPoints: form.storyPoints ? Number(form.storyPoints) : null } as Ticket);
    onClose();
  }
    ...
    <div onClick={onClose} style={{ position: "fixed", inset: 0, background: "rgba(0,0,0,0.4)", display: "flex", alignItems: "center", justifyContent: "center", zIndex: 1000, padding: 20 }}>
      <div onClick={e => e.stopPropagation()} style={{ background: "#fff", borderRadius: 14, width: "100%", maxWidth: 520, padding: 28, boxShadow: "0 20px 50px rgba(0,0,0,0.18)" }}>
        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 20 }}>
          <h2 style={{ margin: 0, fontSize: 17, fontWeight: 700, color: "#111827" }}>Edit Ticket</h2>
          <button onClick={onClose} style={{ background: "none", border: "none", fontSize: 20, cursor: "pointer", color: "#9CA3AF" }}>×</button>
        </div>
        
        <div style={{ display: "flex", flexDirection: "column", gap: 14 }}>
          <input autoFocus value={form.title} onChange={e => setForm((f: any) => ({...f, title: e.target.value}))} placeholder="Ticket title *" style={{ border: "1.5px solid #E5E7EB", borderRadius: 8, padding: "10px 12px", fontSize: 14, fontWeight: 500, outline: "none", width: "100%", boxSizing: "border-box" }} onKeyDown={e => { if (e.key === "Enter") handleSave(); }} />
          ...
          <div style={{ display: "flex", gap: 10, marginTop: 4 }}>
            <button onClick={onClose} style={{ flex: 1, padding: "11px", background: "#fff", color: "#374151", border: "1px solid #E5E7EB", borderRadius: 8, fontSize: 14, fontWeight: 600, cursor: "pointer" }}>Cancel</button>
            <button onClick={handleSave} style={{ flex: 1, padding: "11px", background: "#4F46E5", color: "#fff", border: "none", borderRadius: 8, fontSize: 14, fontWeight: 600, cursor: "pointer" }}>Save Changes</button>
          </div>
        </div>
      </div>
    </div>
